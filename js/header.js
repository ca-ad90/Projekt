class svgDrag2 {
    ...
    */
    ...
    mouseDown(el) {
this.cancelRevert()
        if ((el.target == this.navBtn || $(this.navBtn).find(el.target).length > 0)) {
    ...
    mouseUp(el) {
        if ((el.target == this.navBtn || $(this.navBtn).find(el.target).length > 0) && this.clickEvent) {
            ...
            if (this.type == "drag") {
                this.isDragging = false;
                if (Math.abs(this.qy) > (this.win.height * 0.1) && Math.abs(this.dragLen) > 150) {
                    if (!this.isOpen) {
                        this.open();
                    } else if (this.isOpen) {
                        this.close();
        ...
        if (this.clickEvent) {
            ...
            var openLoop = (timestamp) => {
                ...
                if (elapsed > openMs || this.path.l2[1] == to) {
                    ...
                    this.reqOpen = window.requestAnimationFrame(openLoop.bind(this))
                }
            }
    ...
    async close() {
        ...
        var ani = new Promise(async (res) => {
            ...
            var closeLoop = (timestamp) => {
                ...
                let pos = Math.max(to, (this.win.height * 0.75) - ((this.win.height * 0.75) * curve(part)))
                this.path.l2 = this.lxy(0, pos);
                this.svgHeight = this.bar.height + "px"
                this.wrapper.style.height = (this.bar.height + 5) + "px"
    ...
    async revert() {
        ...
        var p = new Promise(async (res) => {
            ...
            const loop = async (timestamp) => {
                ...
                let xpos = Math.max(-this.bar.width / 2, xPos + xLen * curve(xdel))
...
doc.addEventListener('scroll', function (e) {
    if (doc.scrollTop > window.innerHeight/3 && $(".navbar-wrapper").css("display") == "none") {
        svgdrag.svgHeight = "60px"
        $(".navbar-wrapper").css("display", "flex").css("height","65px").hide().fadeIn("slow")

    } else if (doc.scrollTop < window.innerHeight/3 && $(".navbar-wrapper").css("display") != "none") {
